...
public sealed class LdapService
{
    ...
    {
        var bases = _cfg.GetSection("Ldap:UserSearchBases").Get<string[]>() ?? Array.Empty<string>();
        bases = bases.Select(s => (s ?? "").Trim()).Where(s => !string.IsNullOrWhiteSpace(s)).ToArray();
    ...
    public LdapUser AuthenticateAndFetchUser(string username, string password)
    {
        ...
        if (raw.Contains('\\'))
        {
            ...
            cred = new NetworkCredential(parts[1], password, parts[0]);
        }
        else
        {
    ...
    public IEnumerable<LdapUser> FetchAllUsers()
    {
        ...
        {
            throw new Exception("AD Sync failed: invalid BindUser/BindPassword (LDAP error 49).", ex);
        }
